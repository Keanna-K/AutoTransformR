...
returned_arr_1b <- mirror(bw_test_img_str, "horizontal")
returned_arr_1c <- mirror(test_img_str, "vertical")
returned_arr_1d <- mirror(bw_test_img_str, "vertical")
...
test_that("Test for correct output dimensions", {
  ...
  expect_equal(dim(returned_arr_1d)[length(dim(returned_arr_1d))], 2)  # should return 2 images (original, vert), testing b&w image
...
test_that("Test that horizontal and vertical mirroring are occuring when specified", {
    ...
    expect_equal(returned_arr_1b[,1,2], bw_test_img[,ncol(bw_test_img)])  # check horizontal mirroring occurs in b&w photo

    expect_equal(returned_arr_1c[1,,,2], test_img[nrow(test_img),,])  # check vertical mirroring occurs in colour photo
    expect_equal(returned_arr_1d[1,,2], bw_test_img[nrow(bw_test_img),])  # check vertical mirroring occurs in b&w photo
